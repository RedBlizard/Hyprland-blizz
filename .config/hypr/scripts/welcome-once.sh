...
launch_welcome_app() {
    ...
    # Check if a tiling window is open and move Yad dialog behind it
    while true; do
        # Get the ID of the focused window
        focused_window=$(xdotool getwindowfocus)

        # Check if the focused window is in tiling mode
        if xprop -id $focused_window | grep -q "WM_WINDOW_ROLE(STRING) = \"tiling_window_class\""; then
            # Move the Yad dialog behind the tiling window
            xdotool windowmove $yad_pid --sync $focused_window
            break
        fi
    done
}
