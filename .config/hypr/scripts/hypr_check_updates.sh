...
send_notification_dunst() {
    local message="$1"
    local urgency="$2"
    dunstify -u "$urgency" "$message"
}

# Function to send a notification using notify-send
send_notification_notify_send() {
    local message="$1"
    local urgency="$2"
    notify-send --urgency="$urgency" "$message"
}

# Function to send a notification, choosing the method based on availability
send_notification() {
    local message="$1"
    local urgency="$2"
    if command -v dunstify > /dev/null; then
        send_notification_dunst "$message" "$urgency"
    elif command -v notify-send > /dev/null; then
        send_notification_notify_send "$message" "$urgency"
    else
        echo "No notification system available." >&2
    fi
}
