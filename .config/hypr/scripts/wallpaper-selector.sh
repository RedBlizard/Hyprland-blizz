...
cp $selected_background "$HOME/.cache/awww/current_wallpaper.png"

# Apply the selected background using swww
awww img $selected_background  --transition-step 20 --transition-fps 20
...
yad --picture --file-op \
  ...
  --size=fit \
  --geometry=900x400+800+200 \
  --filename="$HOME/.cache/awww/current_wallpaper.png"
