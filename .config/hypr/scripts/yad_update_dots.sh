...
cd "$HOME/.config/waybar/scripts" || { echo "Failed to change to the waybar scripts directory." >&2; exit 1; }
...
# Function to change ownership of symlink to the current user
change_symlink_owner() {
    local symlink=$1
    sudo chown $(whoami) "$symlink"
}

# Create symlinks in /usr/bin for easy access, suppress output and errors
if symlink_exists "/usr/bin/hypr_check_updates"; then
    echo "Symlink already exists. Continuing..."
else
    # Create symlink
    create_symlink "$(pwd)/hypr_check_updates.sh" "/usr/bin/hypr_check_updates"
    
    # Change symlink ownership to the current user
