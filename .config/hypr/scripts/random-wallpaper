...
if ! pgrep -x swww-daemon >/dev/null; then
  echo "Starting awww-daemon at $(date)" >> ~/.cache/awww/loop.log
  awww-daemon & disown
fi
...
while true; do   
    ...
    awww img "$RANDOMPICS" --transition-step 20 --transition-fps 20
    ...
    echo "[$(date)] Changed wallpaper to: $RANDOMPICS" >> ~/.cache/awww/wallpaper.log
