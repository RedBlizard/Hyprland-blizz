...
case "$1" in
    --popup)
        ...
        notify-send -u low -a CPU -i $HOME/.config/swaync/icons/cpu.svg "CPU time (%)" "$top_procs_with_graph"
        ;;
    *)
        ...
        if [ "$1" != "--no-popup" ]; then
            notify-send -u low -a CPU -i $HOME/.config/swaync/icons/cpu.svg "CPU usage per core" "Usage:\n$cpu_usage_per_core%\n\nGraph:\n$graph"
        fi
        ;;
esac
